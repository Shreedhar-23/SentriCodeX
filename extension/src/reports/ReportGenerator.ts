...
export class ReportGenerator {
  ...
  ''
    ...
    if (!findings.length) lines.push(suppressedFindings.length ? 'No active findings.' : 'No findings. Great work!');
    else {
      lines.push('| Severity | Rule ID | File | Line | Description | Recommendation |','|---|---|---|---|---|---|');
      for (const f of sortBySeverity(findings)) lines.push(`| ${capitalize(f.severity)} | ${f.rule_id} | ${escapeMarkdownCell(f.file)} | ${f.line} | ${escapeMarkdownCell(f.description)} | ${escapeMarkdownCell(f.recommendation)} |`);
    }
    lines.push('', '## Suppressed Findings', '');
    if (!suppressedFindings.length) lines.push('No suppressed findings.');
    else {
      lines.push('| Severity | Rule ID | File | Line | Description | Suppression |','|---|---|---|---|---|---|');
      for (const f of suppressedFindings) lines.push(`| ${capitalize(f.severity)} | ${f.rule_id} | ${escapeMarkdownCell(f.file)} | ${f.line} | ${escapeMarkdownCell(f.description)} | ${escapeMarkdownCell(f.suppression_comment)} |`);
    }
...
<strong>${suppressedFileCount}</strong>
<span>Suppressed Files</span>
</div>
<div class="summary-pill"><strong>${findings.length + suppressedFindings.length}</strong><span>Total Detected</span></div></div>
